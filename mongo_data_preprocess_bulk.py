...
def mongo_preprocess(user_to_underwrite,cursor_users,cursor_devices, geolocator):
    ...
    print('DF TO BE HANDLED:\t',df.shape)
    ...
    removal_columns.remove('user_id')
    df = df[df.user_id.isin(user_to_underwrite)].drop(removal_columns,axis=1).drop_duplicates().reset_index(drop=True)
    print(df.shape)
    return df.to_dict()
